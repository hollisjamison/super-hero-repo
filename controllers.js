const { getAllHeroes, getOneHero, addHero } = require('./models')
...
const getAllHeroesController = (request, response) => {
...
const getOneHeroController = (request, response) => {
    // use model to find our hero
    const foundHero = getOneHero(request.params.slug)
...
const addHeroController = (request, response) => {
    ...
    const newHeroes = addHero(newHero)
